...
* I2C master resource using HAL APIs. The I2C master SCB sends the
...
********************************************************************************
* Summary:
* This is a callback event for I2C slave
*
* Parameters:
	...
	if(0UL != (CYHAL_I2C_SLAVE_WR_CMPLT_EVENT & event))
	{
		if(0UL == (CYHAL_I2C_SLAVE_ERR_EVENT & event))
		{
			...
			if ((i2c_slave_read_buffer[PACKET_SOP_POS] == PACKET_SOP) &&
				...
				{
					/* Execute command */
					cyhal_gpio_write((cyhal_gpio_t)CYBSP_USER_LED, i2c_slave_read_buffer[PACKET_CMD_POS]);
				}
		}
...
int main(void)
{
    ...
    printf(">> Configure I2C master \r\n");
    mI2C_cfg.is_slave = false;
    mI2C_cfg.address = 0;
    mI2C_cfg.frequencyhal_hz = 400000;
    ...
    /* Configure I2C Slave */
    printf(">> Configure I2C Slave \r\n\n");
    sI2C_cfg.is_slave = true;
	sI2C_cfg.address = I2C_SLAVE_ADDR;
	sI2C_cfg.frequencyhal_hz = 400000;
	...
	cyhal_i2c_enable_event(&sI2C, CYHAL_I2C_SLAVE_WR_CMPLT_EVENT, 3, true);
	cyhal_i2c_enable_event(&sI2C, CYHAL_I2C_SLAVE_RD_CMPLT_EVENT, 3, true);
	cyhal_i2c_enable_event(&sI2C, CYHAL_I2C_SLAVE_ERR_EVENT		, 3, true);
    ...
    for(;;)
    {
        ...
        if (CY_SCB_I2C_SUCCESS == cyhal_i2c_master_write(&mI2C, I2C_SLAVE_ADDR, buffer, PACKET_SIZE, 0, true))
		{
			...
			if (CY_SCB_I2C_SUCCESS == cyhal_i2c_master_read(&mI2C, I2C_SLAVE_ADDR, buffer, PACKET_SIZE , 0, true))
			{
